#include "download.h"

Download::Download(QObject *parent)
    :QObject(parent)
{
    managerGroup = new QNetworkAccessManager ();
    connect(managerGroup, &QNetworkAccessManager::finished, this, &Download::onResultGroup);

    managerStud = new QNetworkAccessManager ();
    connect(managerStud, &QNetworkAccessManager::finished, this, &Download::onResultStud);

    managerPers = new QNetworkAccessManager ();
    connect(managerPers, &QNetworkAccessManager::finished, this, &Download::onResultPres);
}

void Download::getDataGroup()
{
    QUrl studGroup("http://192.168.0.10/groups"); // Стучимся туда, где расположен файл
//    QUrl studGroup("file:/home/admin1/groups.json"); // Стучимся туда, где расположен файл
//    QUrl studGroup("file:C://Users//Lapte//Documents//groups.json"); // Стучимся туда, где расположен файл
    QNetworkRequest requestGroup;
    requestGroup.setUrl(studGroup);
    managerGroup->get(requestGroup);  // Отправляем запрос
}

void Download::getDataStud(QString comboGroup)
{
    if (comboGroup == "Все группы")
    {
        QUrl stud("http://192.168.0.10/groups/all"); // Стучимся туда, где расположен файл
//        QUrl stud("file:/home/admin1/students.json"); // Стучимся туда, где расположен файл
//        QUrl stud("file:C://Users//Lapte//Documents//321//students.json"); // Стучимся туда, где расположен файл
        QNetworkRequest requestStud;
        requestStud.setUrl(stud);
        managerStud->get(requestStud);  // Отправляем запрос
    }
    else
    {
        QUrl stud("http://192.168.0.10/groups/" + comboGroup); // Стучимся туда, где расположен файл
//        QUrl stud("file:/home/admin1/students.json"); // Стучимся туда, где расположен файл
//        QUrl stud("file:C://Users//Lapte//Documents//" + comboGroup + "//students.json"); // Стучимся туда, где расположен файл
        QNetworkRequest requestStud;
        requestStud.setUrl(stud);
        managerStud->get(requestStud);  // Отправляем запрос
    }

    QUrl studPres("http://192.168.0.10/timerecords"); // Стучимся туда, где расположен файл
//    QUrl studPres("file:/home/admin1/timerecords.json"); // Стучимся туда, где расположен файл
//    QUrl studPres("file:C://Users//Lapte//Documents//timerecords.json"); // Стучимся туда, где расположен файл
    QNetworkRequest requestPres;
    requestPres.setUrl(studPres);
    managerPers->get(requestPres);  // Отправляем запрос
}

void Download::onResultGroup(QNetworkReply *reply)
{
    if (reply->error())
    {
        qDebug() << "Error";
        qDebug() << reply->errorString();
    }
    else
    {
        QFile *questGroup = new QFile ("/home/admin1/group.json"); // Создаем новый файл и копируем туда содержимое полученного файла для дальнейшей обработки
//        QFile *questGroup = new QFile ("C://Users//Lapte//Documents//group.json");
        if(questGroup->open(QFile::WriteOnly))
        {
            questGroup->write(reply->readAll());
            questGroup->close();
        }
        qDebug() << "Download is completed";
        emit onReady();
    }
}

void Download::onResultStud(QNetworkReply *reply)
{
    if (reply->error())
    {
        qDebug() << "Error";
        qDebug() << reply->errorString();

    }
    else
    {
        QFile *questStud = new QFile ("/home/admin1/student.json"); // Создаем новый файл и копируем туда содержимое полученного файла для дальнейшей обработки
//        QFile *questStud = new QFile ("C://Users//Lapte//Documents//student.json");
        if(questStud->open(QFile::WriteOnly))
        {
            questStud->write(reply->readAll());
            questStud->close();
        }
        qDebug() << "Download is completed";
        emit onReady();
    }
}

void Download::onResultPres(QNetworkReply *reply)
{
    if (reply->error())
    {
        qDebug() << "Error";
        qDebug() << reply->errorString();

    }
    else
    {
        QFile *questPres = new QFile ("/home/admin1/timerecord.json"); // Создаем новый файл и копируем туда содержимое полученного файла для дальнейшей обработки
//        QFile *questPres = new QFile ("C://Users//Lapte//Documents//timerecord.json");
        if(questPres->open(QFile::WriteOnly))
        {
            questPres->write(reply->readAll());
            questPres->close();
        }
        qDebug() << "Download is completed";
        emit onReady();
    }
}
