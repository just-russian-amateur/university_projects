        ...
        if(students.isArray())
        {
            ...
            for (int i = 0; i < student.count(); i++) {
                ...
                for (int j = 0; j < timerecord.count(); j++) {
                    ...
                    if (personal.value("id").toInt() == present.value("id").toInt())    // Сравниваем идентификатор студента в списке студентов и посещаемости, чтобы выводить посещаемость напротив конкретного студента
                    {
                        if(present.value("present").toBool() == true)
                            pres = "Присутствовал";
                        else
                            pres = "Отсутствовал";
                        QList<QStandardItem *> items;   // Создаем массив столбцов таблицы и заполняем его данными
                        ...
                        items.append(new QStandardItem(present.value("date").toString()));    // Пятая колонка
                        items.append(new QStandardItem(present.value("time").toString()));    // Шестая колонка
...
{
    comboGroup = ui->comboBox->currentText();   // Получаем строку, выбранную пользователем из выпадающего списка
