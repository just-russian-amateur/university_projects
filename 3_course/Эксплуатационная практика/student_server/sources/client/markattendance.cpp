...
void MarkAttendance::readStud()
{
    ...
    if(students.isArray())
    {
        ...
        ui->tableMark->setItemDelegateForColumn(5, delegate);   // Добавляем солбец с чекбоксами
        ...
        QFile json;
        json.setFileName("/home/time.json");
...
{
    if (reply->error() == QNetworkReply::NoError)   // Проверяем наличие ошибок при выполнении запроса
    {
        qDebug() << "File post";
        reply->deleteLater();
    }
...
void MarkAttendance::postData()
{
    ...
    // Отправляем файл
    QFile *askStud = new QFile ("/home/time.json");
